...
async function processFile(
  ...
  let exist = await Firestore.hasDocument(
    COLLECTION,
    id,
    "author",
    firebaseConfig,
  ...
  if (!exist) {
    const doc = await Firestore.createDocument(
      COLLECTION,
      post,
      id,
      firebaseConfig,
    ...
    const doc = await Firestore.patchDocument(
      COLLECTION,
      post,
      id,
      firebaseConfig,
...
async function storeBlogEntries() {
  ...
  const docs = await Firestore.listDocuments(
    COLLECTION,
    { mask: ["author", "date", "title"], orderBy: "date desc" },
    firebaseConfig,
    token
  );
  console.log(docs);
...
async function registerUser(
  ...
  try {
    ...
    const profileData = await Auth.updateUserProfile(
      userData.idToken,
...
async function registerUsers() {
  await registerUser(
    ...
    "test1234",
    "Alex Alabbas",
    "https://mediatemple.net/blog/wp-content/uploads/2019/10/IMG_8950-400x400.jpg"
  );
